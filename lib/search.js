 ...
 * @return {SearchBuilder}
 */
SearchBuilder.prototype.aggregate = function (aggregate_type, field, options) {
  assert(aggregate_type, 'aggregate type required')
  assert(field, 'field required')
  var _aggregate = 'value.' + field + ':' + aggregate_type;
  options = ":" + options;
  if (options != ":") {
    _aggregate += options;
  }
