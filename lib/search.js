 ...
 * @return {SearchBuilder}
 */
SearchBuilder.prototype.aggregate = function (field, builder) {
  assert(field, 'field required')
  assert(typeof builder !== 'undefined', 'aggregate builder required')

  var _aggregate = 'value.' + field + ':' + builder.getOptions();

  if (this._aggregate) this._aggregate = [this._aggregate, _aggregate].join(',')
  else this._aggregate = _aggregate
  return this
}
