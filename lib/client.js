...
/**
 * Update a sub-document by key value.
 * @param {string} collection
 * @param {string} key
 * @param {Object} data
 * @return {Promise}
 */
Client.prototype.patch = function (collection, key, data) {
  assert(collection && key && data, 'Collection, key and JSON object required.')
 ...
 * @return {Promise}
 */
Client.prototype.remove = function (collection, key, purge, match) {
  assert(collection && key, 'Collection and key required.')
  var header = {}
  header['purge'] = purge
  if (typeof match == 'string') header['If-Match'] = this._quote(match)
  return this._del(this.generateApiUrl([collection, key], header))
}
